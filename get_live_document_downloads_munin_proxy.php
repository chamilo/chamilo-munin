...
if (!empty($argv[1]) && $argv[1] == 'config') {
    ...
    if (!is_file('/tmp/get_live_document_downloads_config')) {
        @exec(__DIR__.'/get_live_document_downloads_munin.php config');
    }
    readfile('/tmp/get_live_document_downloads_config');
    exit;
}
if (!is_file('/tmp/get_live_document_downloads')) {
    @exec(__DIR__.'/get_live_document_downloads_munin.php');
}
readfile('/tmp/get_live_document_downloads');
