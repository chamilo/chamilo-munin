...
chamilo_registered_sessions - Munin plugin to monitor the number of sessions created on Chamilo portals.
...
The plugin shows the number of sessions at any given time for any
...
//    $output .= "portal$portal.max 11000\n";
  ...
  file_put_contents('/tmp/get_registered_sessions_config',$output);
  exit;
}
...
file_put_contents('/tmp/get_registered_sessions',$output);

function get_sessions($bd, $sub) {
  $match_count=0;
  $connections = array();
  $list = scandir($bd);
  foreach ($list as $dir) {
