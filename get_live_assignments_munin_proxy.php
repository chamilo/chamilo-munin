...
chamilo_live_assignments - Munin plugin to monitor the number of assignments uploaded on Chamilo portals.
...
The plugin shows the number of assignments uploaded at any given time 
...
if (!empty($argv[1]) && $argv[1] == 'config') {
    ...
    if (!is_file('/tmp/get_live_assignments_config')) {
        @exec(__DIR__.'/get_live_assignments_munin.php config');
    }
    readfile('/tmp/get_live_assignments_config');
    exit;
}
if (!is_file('/tmp/get_live_assignments')) {
    @exec(__DIR__.'/get_live_assignments_munin.php');
}
readfile('/tmp/get_live_assignments');
