...
chamilo_live_document_downloads - Munin plugin to monitor the number of documents downloaded/seen on Chamilo portals.
...
The plugin shows the number of documents downloaded/seen at any given time 
...
if (!empty($argv[1]) && $argv[1] == 'config') {
    ...
    $output .= "graph_title Chamilo document downloads v2\n";
    $output .= "graph_args --lower-limit 0\n";
    $output .= "graph_category chamilo\n";
    $output .= "graph_info This graph shows the number of documents downloaded/seen on Chamilo portals over time.\n";
    $output .= "graph_vlabel Documents in last $last_connect_minutes min\n";
...
//    echo "portal$portal.max 500\n";
        $output .= "portal$portal.warning 1000\n";
        $output .= "portal$portal.critical 5000\n";
        $output .= "portal$portal.draw LINE2\n";
    }
    file_put_contents('/tmp/get_live_document_downloads_config', $output);
    exit;
}
...
file_put_contents('/tmp/get_live_document_downloads', $output);
        ...
        if (!empty($config_file) && is_file($config_file) && is_readable($config_file)) {
            ...
            if (!empty($_configuration['db_user']) && !empty($_configuration['main_database'])) {
                ...
                if ($inc !== false && $dbh !== false) {
                    ...
                    $row = $res->fetch();
                    $current_date = $row[0];
                    $track_table = 'track_e_downloads';
                    $query = "SELECT count(down_id) ".
                        " FROM ".$track_table.
                        " WHERE DATE_ADD(down_date, ".
